...
for idx in {1..3}; do
  for deploy in master dist hydrodist hds hdvar; do
    for obs in 0.05 0.1 0.2; do
      python train_ratio.py --epoch 2000 --adj binary --tag ms --deploy $deploy --wds anytown --obsrat $obs --batch 200 --run_id $idx
      python train_ratio.py --epoch 2000 --adj binary --tag ms --deploy $deploy --wds ctown --obsrat $obs --batch 120 --run_id $idx
      python train_ratio.py --epoch 2000 --adj binary --tag ms --deploy $deploy --wds richmond --obsrat $obs --batch 50 --run_id $idx
    done
  done
done
